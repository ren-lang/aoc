var $5swOt$fs = require("fs");
var $5swOt$path = require("path");
var $5swOt$process = require("process");
...
var parcelRequire = $parcel$global["parcelRequirefa14"];
if (parcelRequire == null) {
  ...
  $parcel$global["parcelRequirefa14"] = parcelRequire;
}
parcelRequire.register("h7VGm", function(module, exports) {
(function(scope) {
...
*/ var _Scheduler_working = false;
        ...
        return {
            ...
            imports: A2($elm$core$List$map, function(_v1) {
                ...
                return {
                    bindings: bindings,
                    name: name,
                    path: A2($elm$core$String$startsWith, '.', path) ? path + '.ren.mjs' : stdlib + ('/' + (path + '.ren.mjs'))
                };
            }, imports)
        };
    });
    ...
    _Platform_export({
        'Main': {
            ...
            }, A2($elm$json$Json$Decode$field, 'stdlib', $elm$json$Json$Decode$string)))({
                "versions": {
                    "elm": "0.19.1"
                },
                "types": {
...
var $h7VGm = parcelRequire("h7VGm");



const $503e9ca78180412c$var$entry = $5swOt$path.resolve($5swOt$process.cwd(), 'aoc');
const $503e9ca78180412c$var$stdlib = $5swOt$path.join($503e9ca78180412c$var$entry, 'stdlib');
const $503e9ca78180412c$var$compiler = $h7VGm.Elm.Main.init({
    flags: {
        argv: $5swOt$process.argv.slice(2),
        stdlib: $503e9ca78180412c$var$stdlib
    }
});
const $503e9ca78180412c$var$paths = function gatherSourceFiles(dir) {
    return $5swOt$fs.readdirSync(dir, {
        withFileTypes: true
    }).flatMap((entry)=>{
        const path = $5swOt$path.resolve(dir, entry.name);
        return entry.isDirectory() ? gatherSourceFiles(path) : path;
    }).filter((path)=>$5swOt$path.extname(path) === '.ren'
    );
}($503e9ca78180412c$var$entry);
$503e9ca78180412c$var$compiler.ports.fromFs.send({
    $: 'GotProject',
    0: Object.fromEntries($503e9ca78180412c$var$paths.map((path)=>[
            path,
            $5swOt$fs.readFileSync(path, {
                encoding: 'utf8'
            })
        ]
    ))
});
$503e9ca78180412c$var$compiler.ports.toFs.subscribe(({ $: $1 , ...data1 })=>{
    const [year, day] = $5swOt$process.argv.slice(2);
    switch($1){
        case 'WriteFiles':
            {
                ...
                Object.entries(files).forEach(([path, { $: $ , ...data }])=>{
                    switch($){
                        case 'Ok':
                            {
                                const name = `${path}.mjs`;
                                $5swOt$fs.writeFileSync(name, data.src, {
                                    encoding: 'utf8'
                                });
